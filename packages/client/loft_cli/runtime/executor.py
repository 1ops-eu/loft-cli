...
class Executor:
    ...
    def _execute_step(self, step: Step, dry_run: bool) -> StepResult:
        ...
        try:
            from loft_cli_core.registry.executors import get_step_handler
    ...
    def _execute_gate(self, step: Step) -> StepResult:
        ...
        """
        from loft_cli.checks.ssh import check_ssh_reachable
        ...
        if step.command and step.command.startswith("tunnel_ssh_gate:"):
            from loft_cli.local.tunnel import tunnel_down, tunnel_up
            ...
            if check.passed:
                ...
                # it up again with `loft-cli tunnel up <host>`
    ...
    def _execute_compose_health_check(self, step: Step) -> StepResult:
        ...
        from loft_cli.checks.compose import check_compose_health
    ...
    def _execute_verify(self, step: Step) -> StepResult:
        ...
        if step.command == "goss_unavailable":
            self._console.print(
                ...
                "   Ensure loft_cli.goss.generator can import and run cleanly.\n"
            )
            return StepResult(
                step_index=step.index,
                step_id=step.id,
    ...
    def _execute_postflight_check(self, step: Step) -> StepResult:
        ...
        try:
            if check_type == "ssh_reachable":
                ...
                from loft_cli.checks.ports import check_port_open

                host, port_str = params.split(":", 1)
                check = check_port_open(host, int(port_str))

            elif check_type == "wireguard_up":
                from loft_cli.checks.wireguard import check_wireguard_up
                ...
                from loft_cli.checks.container import check_container_running
                ...
                from loft_cli.checks.nginx import check_nginx_ready
    ...
    def _execute_goss_validate(self, step: Step) -> StepResult:
        ...
        from loft_cli.addons.goss.renderer import render_goss_results
        from loft_cli.addons.goss.shipper import ship_and_run
    ...
    def _execute_local_file_write(self, step: Step) -> StepResult:
        ...
        if step.id == "write_local_ssh_conf_d" and self._spec and self._ctx:
            ...
            if hasattr(spec, "wireguard") and spec.wireguard.enabled and spec.wireguard.address:
                ...
                tunnel_comment = f"# Requires: loft-cli tunnel up {spec.host.name}"
    ...
    def _execute_local_command(self, step: Step) -> StepResult:
        ...
        from loft_cli.local.ssh_config import backup_ssh_config, ensure_include
