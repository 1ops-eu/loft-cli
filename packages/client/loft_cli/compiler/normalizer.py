...
from loft_cli_core.utils.files import expand_path, resolve_path
...
def normalize(spec, spec_dir: Path | None = None) -> NormalizedContext:
    ...
    from loft_cli_core.registry import get_normalizer, load_addons
...
def _apply_state_dir(spec) -> None:
    ...
    1. LOFT_CLI_STATE_DIR environment variable
    ...
    env_state_dir = os.environ.get("LOFT_CLI_STATE_DIR")
...
def _normalize_file_template(spec: FileTemplateSpec, ctx: NormalizedContext) -> None:
    ...
    from loft_cli_core.utils.templates import content_hash, render_template_file
