...
def plan(ctx: NormalizedContext) -> Plan:
    ...
    from loft_cli_core.registry import get_planner, load_addons
...
def _plan_bootstrap(spec: BootstrapSpec, ctx: NormalizedContext) -> list[Step]:
    from loft_cli.runtime.steps import bootstrap as bs
    from loft_cli.runtime.steps import wireguard as wg
    ...
    if spec.wireguard.enabled:
        ...
        # Client config: saved locally to ~/.wg/loft-cli/{host}/client.conf
    ...
    if goss_content is not None:
        steps.append(
            _s(
                ...
                R,
                StepKind.SSH_COMMAND,
                command=f"mkdir -p /home/{spec.admin_user.name}/.goss",
                sudo=False,
                tags=["goss"],
            )
        )
...
def _plan_service(spec: ServiceSpec, ctx: NormalizedContext) -> list[Step]:
    from loft_cli.runtime.steps import container as ct
    from loft_cli.runtime.steps import docker as dk
    from loft_cli.runtime.steps import nginx as nx
    from loft_cli.runtime.steps import postgres as pg
...
def _plan_file_template(spec: FileTemplateSpec, ctx: NormalizedContext) -> list[Step]:
    ...
    from loft_cli.runtime.steps import file_template as ft
...
def _plan_compose_project(spec: ComposeProjectSpec, ctx: NormalizedContext) -> list[Step]:
    ...
    from loft_cli.runtime.steps import compose as cp
    from loft_cli.runtime.steps import docker as dk
...
def _plan_backup_job(spec: BackupJobSpec, ctx: NormalizedContext) -> list[Step]:
    ...
    b = spec.backup
    src = b.source
    script_name = f"loft-cli-backup-{b.name}"
    ...
    if lr and lr.enabled:
        ...
        steps.append(
            _s(
                ...
                target_path=f"/etc/logrotate.d/loft-cli-{u.unit_name}",
                sudo=True,
