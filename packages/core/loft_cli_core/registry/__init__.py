"""loft-cli addon/plugin registry.
...
during registration to extend loft-cli without modifying any core source files.

Built-in kinds (bootstrap, service) are registered by the ``loft-cli`` client
package via the ``loft_cli.addons`` entry_points group.
...
External addons also register via Python entry_points:
    ...
    def register():
        from loft_cli_core.registry import register_spec_kind, register_planner, ...
...
from loft_cli_core.registry.normalizers import get_normalizer, register_normalizer
from loft_cli_core.registry.planners import get_planner, register_planner
from loft_cli_core.registry.resolvers import get_resolver, list_resolvers, register_resolver
from loft_cli_core.registry.specs import get_spec_model, list_spec_kinds, register_spec_kind
from loft_cli_core.registry.validators import get_validator, register_validator
...
def load_addons() -> None:
    ...
    # The loft-cli client package registers built-in kinds (bootstrap, service,
    # file_template, compose_project) via its own entry_point.
    try:
        import importlib.metadata

        entry_points = importlib.metadata.entry_points(group="loft_cli.addons")
        for ep in entry_points:
            try:
                ...
                warnings.warn(
                    f"Failed to load loft-cli addon '{ep.name}': {exc}",
                    stacklevel=2,
                )
    except Exception:
        pass
