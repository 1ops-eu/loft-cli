...
## v0.6.4 -- Provider-Aware SSH Keys + `loft prep` Command ✅

**Goal:** Introduce provider-scoped SSH key management, align all local state with a `{provider}/{host}` hierarchy, and ship a `loft prep` command that generates keys and guides the user through cloud provider setup before the first `apply`.

**Design principle:** Before you can `loft apply`, you need an SSH key on the machine. Today that's a manual `ssh-keygen` + copy-paste-to-cloud-console workflow. `loft prep` formalizes this as the entry point — generate the key, show the public key for deployment, and optionally verify connectivity. Cloud API automation (deploy key via `hcloud ssh-key create`, etc.) is deferred to loft-cli-pro.

### Provider-Aware Local State

The `host.provider` field scopes all local state. This prevents collisions when the same `host.name` exists on multiple cloud providers (e.g. `dev-vps` on both Hetzner and IONOS).

| Item | Description |
|---|---|
| `host.provider` spec field | New optional field on `host:` block. Required when SSH key auto-generation is active. Free-form string (e.g. `hetzner`, `ionos`, `ovh`, `generic`) |
| SSH keys: `~/.loft-cli/keys/{provider}/{host}/id_ed25519` | Per-host, per-provider SSH keypair. Auto-generated by `loft prep` or `apply` when `admin_user.pubkeys` is omitted or set to `auto` |
| WireGuard state: `~/.wg/loft-cli/{provider}/{host}/` | Aligns existing per-host WireGuard state with the provider dimension |
| SSH config: `~/.ssh/conf.d/loft-cli/{provider}--{host}.conf` | SSH Host alias becomes `{provider}--{host}` (e.g. `Host hetzner--dev-vps`). The `--` separator avoids `/` issues in SSH Host directives |
| Inventory: composite key `{provider}/{host}` | Inventory records keyed by provider + host name, not host name alone |
| Backward compatibility | Explicit `pubkeys: [~/.ssh/my_key.pub]` in specs bypasses auto-generation entirely. Provider is only required for auto-managed keys |

**New local state layout (complete):**
...
~/.loft-cli/
    ...
    inventory.db                    # SQLite — composite key: {provider}/{host}
    runs/
~/.ssh/conf.d/loft-cli/
    {provider}--{host}.conf         # SSH config fragment
~/.wg/loft-cli/
    {provider}/
        {host}/
            private.key
...
### `loft prep` Command (OSS)

New CLI command: `loft prep <spec.yaml> [--env-file .env] [--check-connection]`

| Item | Description |
|---|---|
| Phase 1: key generation + display | Parse spec, resolve `host.provider` and `host.name`. Check if SSH keypair exists at `~/.loft-cli/keys/{provider}/{host}/id_ed25519`. If missing, generate it (ed25519, no passphrase). Print the public key to stdout with copy-paste-ready formatting |
| Public key output | Display the full public key content so the user can copy it into their cloud provider console (Hetzner Cloud → SSH Keys, etc.) |
| Next-steps guidance | Print actionable instructions: (1) add key to cloud provider, (2) create/start server with the key, (3) run `loft prep --check-connection` |
| Phase 2: `--check-connection` | Attempt SSH connection to `host.address` using the generated key. Report: key exists? permissions 0600? host reachable? SSH port open? authentication successful? |
| Exit codes | Exit 0 on success. Non-zero with actionable error messages on failure |

**Example output (Phase 1):**

```
SSH keypair ready: hetzner/dev-vps
Path: ~/.loft-cli/keys/hetzner/dev-vps/id_ed25519

Public key (copy to your cloud provider):
────────────────────────────────────────────
ssh-ed25519 AAAAC3Nz... loft-cli:hetzner/dev-vps
────────────────────────────────────────────

Next steps:
1. Add this public key in Hetzner Cloud Console → SSH Keys
2. Create or rebuild the server with this key
3. Verify:    loft prep 01-bootstrap/bootstrap.yaml --check-connection --env-file ../.env
4. Bootstrap: loft apply 01-bootstrap/bootstrap.yaml --env-file ../.env
```
...
| Package | File | Change |
|---|---|---|
| core | `loft_cli_core/specs/schemas/` | Add `provider` field to host schema |
| core | `loft_cli_core/registry/local_paths.py` | New `ssh_key_dir(provider, host)` function; update WireGuard paths |
| core | `loft_cli_core/specs/validators/` | Provider-required validation when auto-generating keys |
| client | `loft_cli/commands/prep.py` (new) | `loft prep` command implementation |
| client | `loft_cli/commands/__init__.py` | Register the new command |
| client | `loft_cli/local/keys.py` | Use `~/.loft-cli/keys/{provider}/{host}/` paths for auto-generation |
| client | `loft_cli/local/ssh_config.py` | `{provider}--{host}` naming for SSH config fragments and Host aliases |
| client | `loft_cli/local/wireguard_store.py` | `{provider}/{host}` nesting for WireGuard state |
| client | `loft_cli/local/remove.py` | Update removal to handle provider-scoped paths |
| client | `loft_cli/inventory/` | Composite key `{provider}/{host}` for host identity |

### Scope Split: OSS vs. Pro

| Feature | Where |
|---|---|
| Per-host keypair generation (`~/.loft-cli/keys/{provider}/{host}/`) | loft-cli OSS |
| `loft prep` command (key gen + pubkey display + connection check) | loft-cli OSS |
| `host.provider` spec field + validation | loft-cli-core OSS |
| Provider-scoped WireGuard, SSH config, inventory | loft-cli OSS |
| Cloud API key deployment (`hcloud ssh-key create`, etc.) | loft-cli-pro |
| Cloud API server provisioning (create server with key) | loft-cli-pro |
| Fully automated prep-to-apply pipeline | loft-cli-pro |

### UX Fixes (from Manufactum POC)

| Item | Description |
|---|---|
| `--env-file` for `loft-cli tunnel` | `tunnel up/down` accepts `--env-file` for consistency with all other commands — host name is often stored in the env file |
| "Did you mean?" for unknown kinds | When an unknown spec kind is used, suggest a mapping (e.g. `kind: docker` → `kind: service` with `docker: enabled: true`) |
| Show valid fields on extra-field errors | When `extra='forbid'` rejects a field, include the list of valid fields for that block inline in the error message |
| Compose file vs template conflict warning | If `compose_file` matches a filename in `project.templates`, emit a warning instead of a misleading "additional properties not allowed" error |
| Full Docker build output in run log | Docker build stderr is no longer truncated — full output captured and available for diagnosis |
| Health check timeout hint | When `healthcheck.timeout` is hit, emit a hint suggesting to increase the timeout with context about slow-starting services |
| Docker Compose deprecation warnings as ⚠ | Known deprecation notices (e.g. obsolete `version:` field) surface as warnings, not step failures |
| No stale file removal warning | When files exist in a managed directory that are not in the current spec, emit a warning so the user knows to clean up manually |

**Acceptance criteria:**

- `loft prep` generates an SSH keypair at `~/.loft-cli/keys/{provider}/{host}/id_ed25519` and displays the public key for copy-paste
- `loft prep --check-connection` verifies SSH connectivity to the target host using the generated key
- All local state (SSH keys, WireGuard, SSH config, inventory) uses the `{provider}/{host}` hierarchy
- Same `host.name` on different providers does not collide in any local state
- Existing specs with explicit `pubkeys` paths continue to work unchanged (backward compatible)
- `ssh {provider}--{host}` works via the generated SSH conf.d entry after bootstrap
- `loft-cli remove` cleans up provider-scoped state correctly
- All UX fixes are covered by tests
...
## v0.10 -- Multi-Host Light Operations

**Goal:** Practical small-fleet operations from a single loft-cli command.
