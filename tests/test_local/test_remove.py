...
class TestRemoveHost:
    @patch("loft_cli.local.tunnel.tunnel_down", return_value=(True, "Tunnel not active"))
    ...
    @patch("loft_cli.local.tunnel.tunnel_down", return_value=(True, "Tunnel wg-test is down"))
    ...
    def test_inventory_skipped_when_no_record(self, _mock_down, tmp_path):
        ...
        with patch.dict(os.environ, {"LOFT_CLI_DB_PATH": db_path}):
            console = MagicMock()
            results = remove_host("testhost", console=console)
